...
def test_check_user_succesful(mock_user: User, db_session: Session):
    utente_esistente = Utente(
        username=mock_user.username, password=mock_user.password, descrizione="test"
    )
...
def test_check_create_user_succesful(mock_user: User, db_session: Session):
    service = UserService(db=db_session)
    user = User(username=mock_user.username, password=mock_user.password)

    result = service.create_user(user)

    assert result is True

    statement = select(Utente).where(Utente.username == mock_user.username)
    user_in_db = db_session.exec(statement).first()

    assert user_in_db is not None, "L'utente dovrebbe essere presente nel database"
    assert user_in_db.username == mock_user.username


def test_check_create_user_fail(mock_user: User, db_session: Session):
    utente_esistente = Utente(
        username=mock_user.username, password=mock_user.password, descrizione="test"
    )
    ...
    service = UserService(db=db_session)
    assert service.create_user(mock_user) is False, "Non dovrebbe creare l'utente"
